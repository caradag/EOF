function results = get_cov_matrices(yearstart, yearend, tstartinyear,...
    tendinyear, t_interval, dt, max_nan, max_succ_nan, interp_meth,...
    normalize)
...
% Inputs: yearstart, - strings, the starting and ending year over which 
...
%         (For inputs dt to interp_meth, see extract_v4)
%         dt - scalar in days; default is 2 minutes if not specified
%         max_nan - maximum number of NaN's tolerated in the pressure 
%                   series for removal; default is 10 if not specified 
%         max_succ_nan - maximum number of successive NaN's tolerated in  
%                        the pressure series for removal; default is 3 if 
%                        not specified
...
if nargin < 10 || isempty(normalize);
    normalize = false;
end
...
for kk=1:N2
    ...
    for jj=1:N1;
        ...
        for ii=1:sensor_count;
            ...
            p_in = p_in{1};
            tlim = eval(strcat('data.',sensors{ii},'.limits.time'));
            ...
            if tstart<tlim(1) || tfinal>tlim(2);
                ...
                % Calling extract_v4
                [p_out, t_out] = extract_v4(tstart, tfinal, t_in, p_in,...
                    dt, max_nan, max_succ_nan, interp_meth);
                press_mat(ii,:) = p_out.';
            end   
        end
        ...
        % Calling clean
        [sensors_clean, cov_clean] = clean(sensors, cov);
        ...
        save(filename,'cov_clean','press_mean','sensors_clean',...
            'start_time','end_time');
