function results = makecircle_v3(cov_data, yr, wk, thres, norm)
%% v3 uses the lambda test to decide what eigen-related data to save and plot 
%  (filenames end with "(3)")
% Plotting the e/vectors with eigenvalues of the covariance matrix that satisfy the 
% lambda test, then plotting the components of the eigenvector above thres as  
% circles (red for positive and blue otherwise; circle size represents magnitude) on 
% the relative position of sensors map. 
...
%         thres - scalar (0 to 1.0), user-specified. The miniumum absolute value of  
%                 the eigenvector component for which the pressure series will be 
...
for jj=1:n;
    ...
    filename = [num2str(yr) '_' num2str(wk) '_Eigvec ' num2str(jj) ' (3)'];
    ...
    % subplot(2,1,1)) and plotting the pressure series if val(kk)>thres
    ...
    for kk=1:m;
        ...
        % Plotting and saving the pressure series if thres is exceeded
        if abs(eigvec(kk))>thres;
            figure(2);
            plot(t_in, p_in,'b*');
            xlabel('Time/days');
            ylabel('Pressure')
            title(['Pressure Series, sensor ' sensors_clean{kk} ' (' start_time...
                ' to ' end_time ', Map ' num2str(jj) ') (3)']);
    ...
    % Saving the plot
    filename = [num2str(yr) '_' num2str(wk) '_Map ' num2str(jj) ' (3)'];
    saveas(gcf, filename, 'fig');
    clf(gcf);
end
% Saving the variables
filename = ['E_val, e_vec, amp and time' ' (' cov_data(1:end-4) ') (3)'];
